...
// ввод входных параметров задачи
 ...
 int[,] array = new int[m, n]; // создание двумерного массива
 TwoArray(array);
 WriteArray(array);

 int SumStringElements(int[,] array, int i) // метод расчёта суммы элементов массива для каждой строки
 {
 ...
 int minSumString = 0; // поиск минимальной суммы для строки двумерного массива
 ...
 for (int i = 1; i < array.GetLength(0); i++)
 {
   ...
   int InputNumbers(string input) // метод задания входных значений строки
 {
   Console.Write(input);
 ...
 void TwoArray(int[,] array) // метод создания двумерного массива
 {
 ...
 void WriteArray (int[,] array) // метод печати двумерного массива
 {
   for (int i = 0; i < array.GetLength(0); i++)
   {
     for (int j = 0; j < array.GetLength(1); j++)
